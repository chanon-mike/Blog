...
##CONNECT TO DB
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv("DATABASE_URL", default="sqlite:///blog.db")
...
class User(UserMixin, db.Model):
    ...
    #The "author" refers to the author property in the BlogPost and Comment class.
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # check if user has this email in the database
        user = User.query.filter_by(email=email).first()
        # If the email or password not correct
